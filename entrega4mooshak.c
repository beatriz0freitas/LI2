...
typedef enum
{
    NENHUMA = 0,         // uma sequencia vazia deverá ficar com este valor
...
// ATENCAO: usando bitwise and com a mascara 0x000F0 mas não tivemos tempo de experimentar
int naipeDaCarta(wchar_t carta) {
    ...
    // shift de quatro bits para obter o valor do penúltimo digito e  subtrair 10 para termos os valores de 1-4.
    return valor;
}
...
// calcula os últimos números de passos seguidos
...
int calculaReisDaUltimaJogada(SequenciaDeCartas ultimaJogada)
{
    ...
    // se a primeira carta não for rei reorntar 0
...
{
    // se sequenciaDeTamanhoPretendido tem uma com tamanho zero é porque tem uma falha e não da para fazer sequências
...
ListaDeSequenciasDeCartas calculaSequenciasPossiveis(SequenciaDeCartas ultimaSequenciaJogada, SequenciaDeCartas maoDoJogador)
{
    ...
    if (ultimaSequenciaJogada.tipo == NENHUMA)
    {
        ...
        // ordenar conforme os outputs para passar no mooshak
    // as sequências não estão ordenadas pela carta maior mas sim pela ordem de input
