...
SequenciaDeCartas lerSequencia(FILE *fp)
{
    wchar_t linha[1000];
...
ListaDeSequenciasDeCartas lerListaDeSequencias(FILE *fp, int numeroJogadasAnteriores)
{
    ListaDeSequenciasDeCartas jogadasAnteriores;
    initListaDeSequenciasDeCartas(&jogadasAnteriores, 50);

    for (int i = 0; i < numeroJogadasAnteriores; i++)
    {
        wchar_t linhaDaJogada[1000];
        assert(fgetws(linhaDaJogada, 1000, fp) != NULL);

        SequenciaDeCartas sequenciaDaJogada;
        if (wcscmp(linhaDaJogada, L"PASSO\n") == 0)
        {
            initSequenciaDeCartas(&sequenciaDaJogada, 10);
            sequenciaDaJogada.tamanho = 0;
            sequenciaDaJogada.tipo = NENHUMA;
        }
        else
        {
        ...
        adicionarSequencia(&jogadasAnteriores, sequenciaDaJogada);
    }
...
int numeroDePassos(ListaDeSequenciasDeCartas jogadas)
{
    int numeroDePassos = 0;
    for (int i = 1; i <= jogadas.tamanho; i++)
    {
        if (jogadas.sequencias[jogadas.tamanho - i].tamanho == 0)
        {
            numeroDePassos++;
        }
        else
...
SequenciaDeCartas calcularUltimaSequenciaJogada(ListaDeSequenciasDeCartas jogadasAnteriores)
{
    SequenciaDeCartas ultimaSequenciaJogada;
    int quantidadeDePassou = numeroDePassos(jogadasAnteriores);

    if (quantidadeDePassou >= 3)
    {
        initSequenciaDeCartas(&ultimaSequenciaJogada, 10);
        ultimaSequenciaJogada.tamanho = 0;
        ultimaSequenciaJogada.tipo = NENHUMA;
    }
    else
    {
        ultimaSequenciaJogada = jogadasAnteriores.sequencias[jogadasAnteriores.tamanho - quantidadeDePassou - 1];
    }

    return ultimaSequenciaJogada;
}
...
void removerTodasAsJogadasAnteriores(SequenciaDeCartas *maoDoJogador, ListaDeSequenciasDeCartas jogadasAnteriores)
{
    for (int i = 0; i < jogadasAnteriores.tamanho; i++)
    {
        // assume-se que todas as jogadas anteriores ja foram validadas
        // os testes o dirão se é verdade :)
        removerJogada(maoDoJogador, jogadasAnteriores.sequencias[i]);
    }
}

// remover
void removerJogadasAnterioresePredentidas(SequenciaDeCartas *maoDoJogador, ListaDeSequenciasDeCartas jogadasAnteriores, SequenciaDeCartas sequenciaDaJogadaPretendida)
{

    removerTodasAsJogadasAnteriores(maoDoJogador, jogadasAnteriores);
    ...
    if (jogadasAnteriores.tamanho == 0)
    {
    ...
    int numeroDePassosSeguidos = numeroDePassos(jogadasAnteriores);
    ...
    SequenciaDeCartas ultimaJogada = jogadasAnteriores.sequencias[jogadasAnteriores.tamanho - numeroDePassosSeguidos - 1];
...
ListaDeSequenciasDeCartas calculaSequenciasPossiveis(SequenciaDeCartas ultimaSequenciaJogada, SequenciaDeCartas maoDoJogador)
{
    ...
    { 
        // adicionar todos os conjuntos possiveis de pamanho de 1 a 4
        for (int tamanhoDaJogada = 1; tamanhoDaJogada <= 4; tamanhoDaJogada++)
        {
            adicionarTodosOsConjuntosPossiveis(&sequenciasPossiveis, maoDoJogador, tamanhoDaJogada, 0);
        }
        // adicionar todas as sequências possiveis de pamanho de 3, 4 a 14
        // para as duplas de 6 a 28 (6, 8, 10,...)
        for (int tamanhoDaJogada = 3; tamanhoDaJogada < 15; tamanhoDaJogada++)
        {
            adicionarTodasAsSequenciasPossiveis(&sequenciasPossiveis, maoDoJogador, tamanhoDaJogada, 0);
            adicionarTodasAsSequenciasDuplasPossiveis(&sequenciasPossiveis, maoDoJogador, tamanhoDaJogada * 2, 0);
        }
    }
...
void escolheJogada(SequenciaDeCartas ultimaSequenciaJogada, SequenciaDeCartas maoDoJogador)
{

    ListaDeSequenciasDeCartas sequenciasPossiveis;
    sequenciasPossiveis = calculaSequenciasPossiveis(ultimaSequenciaJogada, maoDoJogador);

    // Se não houver sequencias possiveis escreve PASSO
    ...
    SequenciaDeCartas sequenciaEscolhida;
    sequenciaEscolhida = sequenciasPossiveis.sequencias[sequenciasPossiveis.tamanho - 1];

    // escolhe a sequencia de maior tamanho com menor valor
    for (int i = 1; i < sequenciasPossiveis.tamanho; i++)
    {
        if (sequenciaEscolhida.tamanho == sequenciasPossiveis.sequencias[sequenciasPossiveis.tamanho-1-i].tamanho)
        {
            sequenciaEscolhida = sequenciasPossiveis.sequencias[sequenciasPossiveis.tamanho-1-i];
        }
    }

    imprimeSequencia(sequenciaEscolhida);
}

int guiaoAMooshak(FILE *fp)
{

    int linhasInput = lerNumero(fp);
...
int guiaoBMooshak(FILE *fp)
{
    ...
    int quantidadeDeTestes = lerNumero(fp);

    // para cada teste
    for (int i = 0; i < quantidadeDeTestes; i++)
    {
        // ler a quantidade de sequências que vamos ter
        int quantidadeDeSequenciasDoTeste = lerNumero(fp);
...
int guiaoCMooshak(FILE *fp)
{
    ...
    for (int i = 0; i < quantidadeDeTestes; i++)
    {
        ...
        int numeroDeJogadas = lerNumero(fp);
        ...
        // ler jogadas
        ListaDeSequenciasDeCartas jogadasAnteriores = lerListaDeSequencias(fp, numeroDeJogadas);
        ...
        removerJogadasAnterioresePredentidas(&maoDoJogador, jogadasAnteriores, sequenciaDaJogadaPretendida);
...
int jogador(FILE *fp)
{
    ...
    // ler linha com número de jogadas anteriores
    int numeroJogadasAnteriores = lerNumero(fp);

    // ler mão do jogador
    SequenciaDeCartas maoDoJogador = lerSequencia(fp);

    // ler jogadas anteriores
    ListaDeSequenciasDeCartas jogadasAnteriores = lerListaDeSequencias(fp, numeroJogadasAnteriores);

    // calcular a última sequência jogada
    SequenciaDeCartas ultimaSequenciaJogada = calcularUltimaSequenciaJogada(jogadasAnteriores);

    // remover todas as jogadas anteriores da mão do jogador
    removerTodasAsJogadasAnteriores(&maoDoJogador, jogadasAnteriores);

    // escolher e jogar
    escolheJogada(ultimaSequenciaJogada, maoDoJogador);
...
int main(int argc, char *argv[])
{
    ...
    int result = jogador(origemDoInput);
